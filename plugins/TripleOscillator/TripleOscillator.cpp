...
void OscillatorObject::oscUserDefWaveDblClick()
{
	...
	if( af != "" )
	{
		if (const auto buffer = SampleBuffer::fromFile(af))
		{
...
void TripleOscillator::loadSettings( const QDomElement & _this )
{
	for( int i = 0; i < NUM_OF_OSCILLATORS; ++i )
	{
		...
		if (auto userWaveFile = _this.attribute("userwavefile" + is); !userWaveFile.isEmpty())
		{
			...
			{
				if (const auto buffer = SampleBuffer::fromFile(userWaveFile))
				{
					m_osc[i]->m_sampleBuffer = *buffer;
					m_osc[i]->m_userAntiAliasWaveTable
						= Oscillator::generateAntiAliasUserWaveTable(&m_osc[i]->m_sampleBuffer);
				}
			}

...
{
	if (const auto buffer = SampleBuffer::fromFile(file)) { m_originalSample = Sample{buffer.value()}; }
...
void SlicerT::loadSettings(const QDomElement& element)
{
	if (auto srcFile = element.attribute("src"); !srcFile.isEmpty())
	{
		...
		{
			if (const auto buffer = SampleBuffer::fromFile(srcFile))
			{
				m_originalSample = Sample{buffer.value()};
			}
		}
		else
		{
	...
	{
		if (const auto buffer = SampleBuffer::fromBase64(sampleData, Engine::audioEngine()->outputSampleRate()))
		{
			m_originalSample = Sample{buffer.value()};
		}
	}

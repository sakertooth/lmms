...
void SampleClip::setSampleBuffer(const SampleBuffer& buffer)
{
	{
...
void SampleClip::setSampleFile(const QString& sf)
{
	...
	if (!sf.isEmpty())
	{
		if (const auto buffer = SampleBuffer::fromFile(sf)) { m_sample = Sample{buffer.value()}; }
		updateLength();
	}
	else
	{
...
void SampleClip::loadSettings( const QDomElement & _this )
{
	...
	if( sampleFile().isEmpty() && _this.hasAttribute( "data" ) )
	{
		...
		if (const auto buffer = SampleBuffer::fromBase64(_this.attribute("data"), sampleRate))
		{
			m_sample = Sample{buffer.value()};
		}
	}
